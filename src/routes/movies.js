...
// --- "did you mean" ---------------------------------------------------------
// A one-character typo in a query param is the most common way to be quietly
// wrong, so the 400 names the nearest thing that would have worked.
...
const didYouMean = (word, candidates) => {
  ...
  // Only suggest a near miss; an unrelated word gets no misleading hint.
...
/** Express gives `?a=1&a=2` as an array; a filter only ever means one value. */
const single = (raw) => (Array.isArray(raw) ? raw[raw.length - 1] : raw);

/** A param that is present but blank (`?genre=`) is a mistake, not "no filter". */
